...
# Feature selection
from sklearn.feature_selection import SelectKBest  # Supports callable
...
def eval_model(model: Pipeline, dummy: Pipeline, X_test: np.array, y_test: np.array) -> dict:
    ...
    Args:
        ...
        dummy (Pipeline): A dummy to compare against
        X_test (np.array): X data
        y_test (np.array): y data (labels/ground truth)

    Returns:
        dict: evaluation metrics in dictionary format
    """
...
def random_model(X_spec: DependencySpecType, y_spec: DependencySpecType) -> Pipeline:
    ...
    Args:
        X_spec (DependencySpecType): The DependencySpec for X
        y_spec (DependencySpecType): The DependencySpec for y

    Returns:
        Pipeline: A pipeline that may be fitted on (X, y) data
        that conforms to (X_spec, y_spec)
    """
...
def dummy(strategy: str = "median") -> Pipeline:
    ...
    Args:
        strategy (str, optional): What strategy to use. Defaults to 'median'.

    Returns:
        Pipeline: A dummy model
    """
...
class SmartDummy(BaseEstimator):
    """A dummy that predicts whatever X was the last time. Made fore time-series predictions
    where the response variable is a time-offset version of known data"""
